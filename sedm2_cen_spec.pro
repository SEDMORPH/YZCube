...
PRO SEDM2_CEN_SPEC, dir_in, dir_out, tauv,mu_d, $
...
;;-- set up plotting file
  ...
  if n_elements(snap_in) gt 0 then psfile = dir_out+string(cell)+'_spectra'+outstr+'_'+string(snap_in,form='(I3.3)')+string(style)+'.ps' $
...
;;-- outfile (single snapshot, all components, all orientations)
     outfile_fits = dir_out+string(cell)+'_spec'+outstr+'_'+str_snap+string(style)+'.fits'
...
;;-- read simulation files  - 9 secs
 ...
 ;;-- select particles in the center part
      ...
      ;;read the centers
      if cell eq 'cen' then openr, centertxt, dir_in+'centers.txt', /get_lun
      if cell eq 'tracked_cell' then openr, centertxt, dir_in+'mean_median_pos_initial_offset_3_cell_size_2.txt', /get_lun
      ...
      get_center_ind, newstars, cen_part_ind, center=center, box_size=box_size
      newstar_cen_ind = cen_part_ind
      ; print, n_elements(newstar_cen_ind)
      ; print, total(newstar_cen_ind)
      ; print, n_elements(stars)
      print, "Check stars:"
      get_center_ind, stars, cen_part_ind, center=center, box_size=box_size
      ;print, cen_part_ind
      stars = stars[cen_part_ind]
      ; print, n_elements(stars)
      ; print, n_elements(gas)
      print, "Check gas:"
      get_center_ind, gas, cen_part_ind, center=center, box_size=box_size
...
;;------------------------------------------------------------------
     ...
     plot, lambda,spec_tau[*,0],/xlog,/ylog,/xs,xtitle='Wavelength [A]',ytitle=textoidl('Luminosity [L_\odot/A]'),title='Orien 0: Snapshot '+str_snap+' with dust'+' (center region)';+' N*='+string(nstars,form='(I0)')
     ...
     plot, lambda,spec_notau[*,0],/xs,xr=[3000,9000],title='Orien 0: Snapshot '+str_snap+' N*='+string(nstars,form='(I0)')+' (center region)'
     oplot, lambda,spec_os[*,0],color=cgcolor('red')
     oplot, lambda,spec_ns[*,0],color=cgcolor('cyan')
     oplot, lambda,spec_g[*,0],color=cgcolor('purple')

     plot, lambda,spec_tau[*,0],/xs,xr=[3000,9000],title='Orien 0: Snapshot '+str_snap+' with dust'+' N*='+string(nstars,form='(I0)')+' (center region)'
     oplot, lambda,spec_os_dust[*,0],color=cgcolor('red')
     oplot, lambda,spec_ns_dust[*,0],color=cgcolor('cyan')
     oplot, lambda,spec_g_dust[*,0],color=cgcolor('purple')
